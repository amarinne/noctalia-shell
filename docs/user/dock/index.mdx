...
overview_auto_hide  = false      # show only while the compositor overview is open; no pointer reveal
...
Dock settings expose **Off**, **On**, **Smart**, and **Overview** as one control. In config they map to `auto_hide`, `smart_auto_hide`, and `overview_auto_hide`:

- **Off**: `auto_hide = false`, `smart_auto_hide = false`, `overview_auto_hide = false`
- **On**: pointer auto-hide (`auto_hide = true`)
- **Smart**: workspace-based hide (`smart_auto_hide = true`, `auto_hide = false`)
- **Overview**: shown only while the compositor overview is open (`overview_auto_hide = true`, other two `false`)
...
Set auto-hide to **Overview** in dock settings (or `overview_auto_hide = true` with the other two flags `false` in config). The dock is hidden during normal use — the screen-edge trigger strip is removed, so moving the pointer near the dock edge never reveals it — and slides in only while the compositor overview is open.

- Requires compositor overview support (Niri, Triad). On compositors that do not report overview state, the dock stays hidden.
- Uses the same slide animation as the other auto-hide modes.
- `overview_auto_hide` takes precedence over `smart_auto_hide` if both are set in config.
