...
namespace {
  ...
  public:
    FakeBattery(
        ...
        bool registerChargeMethod, bool hideChargeMethodUntilReappearance
    )
        : connection(connection), path(std::move(path)), nativePath(std::move(nativePath)), model(std::move(model)),
          powerSupply(powerSupply),
          chargeMethodRegistered(registerChargeMethod && !hideChargeMethodUntilReappearance) {
      registerObject();
    }
  ...
  public:
    ...
    FakeBattery& addBattery(
        ...
        bool registerChargeMethod = true, bool hideChargeMethodUntilReappearance = false
    ) {
      ...
      batteries.push_back(
          std::make_unique<FakeBattery>(
              *connection, path, "NOCTALIA_TEST_" + id, std::move(model), powerSupply, registerChargeMethod,
              hideChargeMethodUntilReappearance
          )
      );
      {
        std::lock_guard lock(pathsMutex);
  ...
  void drainUntil(
      SystemBus& bus, Predicate&& predicate,
    ...
    noctalia::test::failCheck("timed out waiting for D-Bus state", location);
  }
...
int main() {
...

  {
    ...
    drainUntil(capabilityBus, [&]() { return findBattery(capabilityService, retryBattery.path) == nullptr; });
    retryBattery.reappearWithChargeMethod();
    fake.readdBattery(retryBattery);
