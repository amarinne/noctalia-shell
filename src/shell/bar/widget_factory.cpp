...
std::unique_ptr<Widget> WidgetFactory::create(
    ...
    const std::string& barName, float widgetSpacing, bool enableScroll, DragDropController* sharedDragDropController
) const {
  ...
  if (auto pluginEntry = scripting::PluginRegistry::instance().resolve(type);
    ...
    auto widget = std::make_unique<PluginWidget>(
        ...
        barName, outputName, sharedDragDropController, enableScroll
    );
    widget->setContentScale(contentScale);
    return widget;
  }
