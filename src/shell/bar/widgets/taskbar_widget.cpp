...
TaskbarWidget::TaskbarWidget(
    ...
    : m_platform(platform), m_configService(config), m_output(output), m_configOptions(std::move(options)),
      ...
      m_activeIndicatorColor(m_configOptions.activeIndicatorColor),
...
TaskbarWidget::~TaskbarWidget() { std::erase(taskbarWidgetInstances(), this); }
...
void TaskbarWidget::finishTaskDrag(float localX, float localY) {
    ...
    for (TaskbarWidget* widget : taskbarWidgetInstances()) {
      ...
      for (const auto& target : widget->m_dragDropTargets) {
        ...
        if (target.node == nullptr
            || target.workspaceId.empty()
            || hasWindow != requireWindow
  ...
  std::vector<std::string> args{
      m_dragDropCommand,   "drop",       "--source-window-id",     sourceWindowId, "--target-workspace-id",
      target->workspaceId, "--fraction", std::to_string(fraction),
  };
  ...
  auto createTaskTile = [&](TaskRef taskRef, std::vector<TaskRef> cycleCandidates = {}, std::string cycleKey = {},
    ...
    if (task.firstHandle != nullptr
      ...
      area->setOnClick([this, taskRef, areaPtr, cycleCandidates = std::move(cycleCandidates),
        ...
        if (data.button == BTN_LEFT) {
          if (consumeSuppressedTaskClick(current->workspaceWindowId)) {
            return;
          }
          if (!cycleCandidates.empty()) {
            std::size_t& cursor = m_groupedAppCycleCursor[cycleKey];
...
void TaskbarWidget::updateModels() {
  ...
  const ModelComparison comparison =
      compareModels(m_showWindowTitle || compositors::isKde(), m_tasks, m_workspaces, nextTasks, nextWorkspaces);
...
bool TaskbarWidget::activeWorkspaceIndex(std::size_t& index) const {
  ...
  for (std::size_t i = 0; i < workspaces.size(); ++i) {
    if (workspaces[i].workspace.active && workspaces[i].hostOutput == effectiveOutput()) {
      index = i;
      return true;
    }
  }
...
wl_output* TaskbarWidget::toplevelOutputFilter() const noexcept {
  return m_showAllOutputs ? nullptr : effectiveOutput();
}
